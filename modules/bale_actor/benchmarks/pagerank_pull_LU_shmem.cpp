...
double pagerank_shmem(sparsemat_t* L, double* page_rank, double* lpage_rank, double* page_rank_curr_iter) {
    // Start timing
    ...
    while (true) {
        ...
        for (int64_t y = 0; y < L->lnumrows; y++) {
            for (int64_t k = L->loffset[y]; k < L->loffset[y+1]; k++) {
                ...
                double pr_y = myshmem_double_g(page_rank, L->lnonzero[k]);
                int64_t deg_y_upper = myshmem_int64_g(L->offset, L->lnonzero[k] + THREADS);
                int64_t deg_y_lower = myshmem_int64_g(L->offset, L->lnonzero[k]);
        ...
        error_all_pes = myshmem_reduce_add(error_current);
        shmem_barrier_all();
        //printf("error: %f\n", error_all_pes);
...
*/
    ...
    double* page_rank = (double*)shmem_malloc(lpr_size*sizeof(double));
    double* lpage_rank = page_rank;
    ...
    //int64_t* degrees_total = (int64_t*)shmem_malloc(lpr_size* sizeof(int64_t));
    ...
    laptime_pagerank = pagerank_shmem(A2, page_rank, lpage_rank, page_rank_curr_iter);
    shmem_barrier_all();
    ...
    total_sum_pageranks = myshmem_reduce_add(sum_pageranks);
