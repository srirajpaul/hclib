...
double tricount_shmem(int64_t *count, sparsemat_t * L, int64_t* vertex_tri_count, int64_t* lvertex_tri_count) {
  int64_t cnt=0;
  ...
  for(l_i = 0; l_i < L->lnumrows; l_i++){ 
    for(k = L->loffset[l_i] + 1; k < L->loffset[l_i + 1]; k++) {
      ...
      for(kk = kbegin; kk < kend; kk++){
        w = myshmem_int64_g(L->nonzero, L_j%THREADS + kk*THREADS); if (w >= L->numrows) w = w - L->numrows; // conversion due to negating triangle neighbor nnz's
        ...
        for(ii = start; ii < L->loffset[l_i + 1]; ii++) {
          ...
          if( w == nonzero_ii ){ 
            ...
            myshmem_fetch_and_inc(vertex_tri_count, L_j + L_j%THREADS); //vertex v is possibly remote 
            myshmem_fetch_and_inc(vertex_tri_count, w + w%THREADS); // vertex w is possibly remote
            ...
            for(int64_t bv = kbegin; bv < kend; bv++){
              int64_t nnz_v = myshmem_int64_g(L->nonzero, L_j%THREADS + bv*THREADS);
              if ((l_i * THREADS + MYTHREAD) == nnz_v || w == nnz_v) {
                myshmem_fetch_and_add(L->nonzero, L_j%THREADS + bv*THREADS, L->numrows);
              }
            }

            // update tri_N(w)
            // loop through w's nonzeros, first_neigh is u, second_neigh is v
            int64_t wbegin = myshmem_int64_g(L->offset, w); 
            int64_t wend   = myshmem_int64_g(L->offset, w + THREADS);
            for(int64_t bw = wbegin; bw < wend; bw++){
              int64_t nnz_w = myshmem_int64_g(L->nonzero, w%THREADS + bw*THREADS);
              if ((l_i * THREADS + MYTHREAD) == nnz_w || L_j == nnz_w) {
                myshmem_fetch_and_add(L->nonzero, w%THREADS + bw*THREADS, L->numrows);
              }
            }
...
double tricent_shmem(int64_t total_tri_cnt, double* tricent_counts, sparsemat_t* L, int64_t* vertex_tri_count, int64_t* lvertex_tri_count) {
  // Start timing
  ...
  for (int64_t v = 0; v < L->lnumrows; v++) {
    for (int64_t k = L->loffset[v]; k < L->loffset[v+1]; k++) {
      ...
      if (u >= L->numrows) {
        ...
        u = u - L->numrows;
        int64_t tri_count_neigh = myshmem_int64_g(vertex_tri_count, u);
        tricent_counts[v] += (((double)1/3) * ((double)tri_count_neigh)) / (double)total_tri_cnt;
      } else {
        // nnz is in N(v)\tri_N(v), update (1)*T(u) / T(G)
        int64_t tri_count_non_neigh = myshmem_int64_g(vertex_tri_count, u);
...
*/
  ...
  int64_t* vertex_tri_count = (int64_t*)shmem_malloc(ltc_size * sizeof(int64_t));
  int64_t* lvertex_tri_count = vertex_tri_count;
  ...
  T0_fprintf(stderr, "\nrunning tricount (shmem) ... \n");
  laptime_tricount = tricount_shmem(&tri_cnt, A2, vertex_tri_count, lvertex_tri_count);
  shmem_barrier_all();
  ...
  T0_fprintf(stderr, "\nrunning tricent (shmem) ... \n");
  laptime_tricent = tricent_shmem(total_tri_cnt, tricent_counts, A2, vertex_tri_count, lvertex_tri_count);
  shmem_barrier_all();
