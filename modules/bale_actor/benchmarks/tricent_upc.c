...
double tricount_upc(int64_t *count, sparsemat_t * L, shared int64_t* vertex_tri_count, int64_t* lvertex_tri_count) {
  int64_t cnt=0;
  ...
  for(l_i = 0; l_i < L->lnumrows; l_i++){ 
    for(k = L->loffset[l_i] + 1; k < L->loffset[l_i + 1]; k++) {
      ...
      for(kk = kbegin; kk < kend; kk++){
        w = L->nonzero[L_j%THREADS + kk*THREADS]; if (w >= L->numrows) w = w - L->numrows; // conversion due to negating triangle neighbor nnz's
        ...
        for(ii = start; ii < L->loffset[l_i + 1]; ii++) {
          ...
          if( w == nonzero_ii ){ 
			...
			myupc_fetch_and_inc(vertex_tri_count, L_j + L_j%THREADS); //vertex v is possibly remote
			myupc_fetch_and_inc(vertex_tri_count, w + w%THREADS);
            ...
            for(int64_t bv = kbegin; bv < kend; bv++){
              int64_t nnz_v = L->nonzero[L_j%THREADS + bv*THREADS];
              if ((l_i * THREADS + MYTHREAD) == nnz_v || w == nnz_v) {
                myupc_fetch_and_add(L->nonzero, L_j%THREADS + bv*THREADS, L->numrows);
              }
            }

            // update tri_N(w)
            // loop through w's nonzeros, first_neigh is u, second_neigh is v
            int64_t wbegin = L->offset[w]; 
            int64_t wend   = L->offset[w + THREADS];
            for(int64_t bw = wbegin; bw < wend; bw++){
              int64_t nnz_w = L->nonzero[w%THREADS + bw*THREADS];
              if ((l_i * THREADS + MYTHREAD) == nnz_w || L_j == nnz_w) {
                myupc_fetch_and_add(L->nonzero, w%THREADS + bw*THREADS, L->numrows);
              }
            }
...
double tricent_upc(int64_t total_tri_cnt, double* tricent_counts, sparsemat_t* L, shared int64_t* vertex_tri_count, int64_t* lvertex_tri_count) {
  // Start timing
...
*/
  ...
  shared int64_t* vertex_tri_count = upc_all_alloc(tc_size, sizeof(int64_t));
  int64_t* lvertex_tri_count = (int64_t*)(vertex_tri_count + MYTHREAD);
  ...
  T0_fprintf(stderr, "\nrunning tricount (upc) ... \n");
  laptime_tricount = tricount_upc(&tri_cnt, A2, vertex_tri_count, lvertex_tri_count);
  upc_barrier;
  ...
  T0_fprintf(stderr, "\nrunning tricent (upc) ... \n");
  laptime_tricent = tricent_upc(total_tri_cnt, tricent_counts, A2, vertex_tri_count,  lvertex_tri_count);
  upc_barrier;
