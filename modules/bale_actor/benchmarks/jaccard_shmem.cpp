...
double get_edge_degrees(sparsemat_t* L, int64_t* edge_degrees) {
    ...
    for (int64_t y = 0; y < L->lnumrows; y++) {
        for (int64_t k = L->loffset[y]; k < L->loffset[y+1]; k++) {
            ...
            int64_t deg_u_upper = myshmem_int64_g(L->offset, L->lnonzero[k] + THREADS);
            int64_t deg_u_lower = myshmem_int64_g(L->offset, L->lnonzero[k]);
...
double jaccard_shmem(sparsemat_t* A2, double* jaccard_index, int64_t* edge_degrees) {
    // Start timing
    ...
    for (int64_t v = 0; v < A2->lnumrows; v++) {             //vertex v (local)
        ...
        for (int64_t k = A2->loffset[v]; k < A2->loffset[v+1]; k++) {
            ...
            for (int64_t kk = A2->loffset[v]; kk < A2->loffset[v+1]; kk++) {
                ...
                if (x == u) continue;
                int64_t row_begin = myshmem_int64_g(A2->offset, u);
                int64_t row_end = myshmem_int64_g(A2->offset, u + THREADS);
                for (int64_t uk = row_begin; uk < row_end; uk++) {
                    int64_t nonzero_x = myshmem_int64_g(A2->nonzero, u%THREADS + uk*THREADS);
                    if (x == nonzero_x) {
                        jaccard_index[pos]++;
                    }
                }
...
*/
    ...
    T0_fprintf(stderr, "\nRunning Jaccard Index (shmem): \n");

    int64_t ljaccard_size = myshmem_reduce_max(A2->lnnz);
    ...
    laptime_jaccard = jaccard_shmem(A2, jaccard_index, edge_degrees);
    shmem_barrier_all();
