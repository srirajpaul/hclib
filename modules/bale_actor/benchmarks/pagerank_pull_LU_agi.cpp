...
double pagerank_agi(sparsemat_t* L, double* page_rank, double* lpage_rank, double* page_rank_curr_iter) {
    // Start timing
    ...
    while (true) {
        ...
        for (int64_t y = 0; y < L->lnumrows; y++) {
            for (int64_t k = L->loffset[y]; k < L->loffset[y+1]; k++) {
                ...
                int64_t deg_y_upper = lgp_get_int64(L->offset, L->lnonzero[k] + THREADS);
...
*/
    ...
    laptime_pagerank = pagerank_agi(A2, page_rank, lpage_rank, page_rank_curr_iter);
    lgp_barrier();
